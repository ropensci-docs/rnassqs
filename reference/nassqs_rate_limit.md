...
Use this function to configure the rate limiting behavior for API
requests to prevent 429 errors (too many requests). By default, the
package will attempt to make faster requests (5 per second), but will
throttle to 1 request every 3 seconds if Error 429 is encountered.
...
- override:
  ...
  Whether to override the rate limiting. Default is FALSE. Only set to
  TRUE for testing or when you know you won't exceed API limits.

- reset_throttling:

  Whether to reset the throttling state to normal speed. Default is
  FALSE.
